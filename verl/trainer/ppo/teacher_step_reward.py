...
def compute_teacher_step_reward_tensor(
    ...
    """Compute teacher step reward with think-only imitation + length reward (SEQUENCE-LEVEL).

    Sequence-level reward: all tokens in think share the same reward value.
    ...
    for i in range(batch_size):
        ...
        for t in range(think_start, think_end):
            ...
            if teacher_pos < teacher_len and token_id == teacher_token_ids[teacher_pos]:
                total_imitation_reward += position_coef
...
def compute_excess_repeat_mask(
    responses: torch.Tensor,
    response_mask: torch.Tensor,
    reward_model_items: Any,
    tokenizer: Any,
    ...
    """Compute mask that zeros out reward for tokens repeated beyond teacher frequency.

    For each token in student response, if the cumulative count of that token exceeds
    the count in teacher sequence, mask out (set to 0) that position's reward.
    ...
    Returns:
        Mask tensor [batch_size, response_len] with 0.0 for excess positions, 1.0 otherwise
    """
    batch_size, response_len = responses.shape
    device = responses.device
    mask = torch.ones((batch_size, response_len), dtype=torch.float32, device=device)
    ...
    for i in range(batch_size):
        ...
        # Count teacher token frequencies
        teacher_counts = Counter(teacher_token_ids)

        # Track student token counts
        seq_len = response_lengths[i]
        student_counts: dict[int, int] = {}

        for pos in range(seq_len):
            token_id = int(responses[i, pos].item())
            student_counts[token_id] = student_counts.get(token_id, 0) + 1

            # If student count exceeds teacher count, zero out this position
            if student_counts[token_id] > teacher_counts.get(token_id, 0):
                mask[i, pos] = 0.0
...
def compute_teacher_step_proxy_reward(
    ...
    Uses compute_teacher_step_reward_tensor for sequence-level rewards (from clean repo).
    Advantage is computed using token-level RTG GRPO (already in core_algos).
    """
    response_mask = response_mask.to(dtype=torch.float32)
    pi_t = torch.exp(old_log_probs).to(dtype=torch.float32)

    # Use sequence-level teacher step reward (from clean repo)
    ...
    # Apply excess repeat mask: zero out reward for tokens repeated beyond teacher frequency
    excess_repeat_mask = compute_excess_repeat_mask(
        responses=responses,
        response_mask=response_mask,
        reward_model_items=reward_model_items,
        tokenizer=tokenizer,
    ...
    reward = reward * excess_repeat_mask
    ...
    metrics = {
        ...
        "teacher_step_reward/excess_repeat_mask_rate": (
            (1.0 - excess_repeat_mask).sum() / response_mask.sum().clamp_min(1.0)
        ).item(),
    }
